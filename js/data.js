...
function P(brandId, category, name, price, salePrice, opts = {}) {
  ...
  return {
    ...
    tags: opts.tags || [],
    image: opts.image || img(`${brandId}-${category}-${id}`, 500, 600),
    imageHover: opts.imageHover || img(`${brandId}-${category}-h${id}`, 500, 600),
    images: [img(`${brandId}-${category}-${id}-a`, 800, 960), img(`${brandId}-${category}-${id}-b`, 800, 960), img(`${brandId}-${category}-${id}-c`, 800, 960), img(`${brandId}-${category}-${id}-d`, 800, 960)],
...
const PRODUCTS = [
  ...
  P('musicm-standard', 'top',    '오버핏 코튼 티셔츠', 29000, 19900, { badge: 'BEST', colors: ['블랙','화이트','그레이','네이비','아이보리'], tags:['베이직','코튼100'], }),
  ...
  // ===== Nike =====
  P('nike', 'shoes', '에어포스 1 07 로우', 139000, null, { badge:'BEST', colors:['올화이트','블랙','트리플블랙'], sizes:['230','240','250','260','270','280'], tags:['스니커즈'], }),
  ...
  // ===== Adidas =====
  P('adidas', 'shoes', '삼바 OG', 149000, null, { badge:'BEST', colors:['블랙/화이트','화이트/검'], sizes:['240','250','260','270','280'], }),
